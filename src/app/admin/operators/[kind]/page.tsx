    ...
    <Container>
      ...
      */}
      ...
      {/*
        ...
        An admin's own address does two jobs and not a third: it signs them in,
        and it is where they are told a submission arrived. It is **not** an
        address to correspond from — replying to a customer from it hands them a
        personal address, and their reply then comes back to that one person
        while `contact@` never sees the rest of the thread (Ben + Aaron,
        2026-09-09, choosing this over the Workspace send-as work).

        It sits here because this is the page where someone adds an admin, which
        is the moment the expectation is set. The same sentence is in the
        contact notification itself, where the decision is actually taken.
      */}
      {kind === "admins" && (
        <p className="mt-3 max-w-[60ch] rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[13px] text-amber-900">
          <strong>Their address signs them in and tells them things — it is not
          for replying to customers.</strong>{" "}
