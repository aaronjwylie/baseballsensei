...
| 1.2.25 | **A sixth multi-recipient message cannot forget the bcc** | `bcc` is required on every sender that accepts more than one address, so omitting it fails `tsc` rather than shipping. Every other message takes `to: string` |
...
| 1.2.20 | **The Admins tab says the address is not for correspondence** | An amber note under the blurb, on that tab only. It is where an admin is added, which is where the expectation is set |
