...
| 1.2.17 | ⚠️ **`contact@` is readable by every admin** | Each admin can open the shared inbox and see the whole correspondence. **Delegated access, not a delivery group** — a group would push mail to admins who muted notifications, which is the one thing the toggle promises it will not do. Workspace configuration, not code |
