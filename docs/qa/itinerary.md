...
| 1.2.17 | ~~⚠️ **`contact@` is readable by every admin**~~ | **Out of scope, 2026-09-09.** Aaron and Ben chose not to take on the Workspace delegation work. Correspondence runs between `contact@` and the customer; an admin's own address signs them in and tells them things, and is not for corresponding from |
| 1.2.19 | **Muting does not cut an admin out of the record.** Untick notify on every admin, then submit the form | `contact@` still receives it; nobody is bcc'd. `site.email` is appended after the notify filter, never through it |
| 1.2.18 | ~~⚠️ **An admin's reply carries the brand address**~~ | **Out of scope, 2026-09-09** — send-as was not taken on. The accepted consequence: a reply from a personal address arrives from that address, so the customer's reply returns to that person alone and `contact@` loses the thread |
| 1.2.26 | ⚠️ **Open question — the contact form has no rate limit.** Post to `sendContactAction` repeatedly from one address | Currently: every one is accepted and mailed. A Server Action is a public endpoint, and the honeypot stops naive bots rather than determined ones; each send costs a Resend credit and lands in five inboxes. **Not the same question as 1.2.11** — that one records the deliberate absence of a *duplicate* guard, which is about a person writing twice, not about abuse. `shared/lib/rateLimit.ts` already guards checkout and both status-code routes, so this would be reuse. **Needs a decision, not a fix** |
| 1.2.24 | ⚠️ **Retest on a fresh tab, not an open one** | Vercel pins a browser to the deployment that served it for 12 hours (Skew Protection), so an open tab keeps running the code that was live when it loaded. ⚠️ Four contact-form tests on 2026-09-09 came out pre-fix while the fix was demonstrably live and every deploy had succeeded. See `qa-plan.md` §5b |
| 1.2.22 | ⚠️ **No admin notice puts the admins in `to`.** Trigger ②, ④, ⑤ and ⑦ and read the headers | `contact@` in `to`, every admin in `bcc`, through one `adminAudience()`. ⚠️ ⑤ handed every admin's personal address to the coach, and the coach's to every admin, until 2026-09-09 |
| 1.2.25 | **A sixth multi-recipient message cannot forget the bcc** | **Covered by `tests/unit/contactReceiptFailure.test.ts`.** `bcc` is required on every sender that accepts more than one address, so omitting it fails `tsc` rather than shipping. Every other message takes `to: string` |
| 1.2.23 | ⑤ still addresses the coach openly | They are in `to` beside `contact@` and not also bcc'd — a notice about your own work should not look like it was sent to somebody else, and both would deliver it twice |
| 1.2.20 | **The Admins tab says the address is not for correspondence** | **Covered by `tests/unit/adminEmailNote.test.tsx`.** An amber note under the blurb, on that tab only. It is where an admin is added, which is where the expectation is set |
| 1.2.21 | **The contact notification says it too** | "Answer from contact@…, not from here" — in the mail, at the moment somebody is deciding whether to hit Reply. A rule that lives only in a document is discovered by breaking it |
| 1.2.14 | The receipt can't fail the form | **Covered by `tests/unit/contactReceiptFailure.test.ts`.** Kill the second send and the form still succeeds. The message is already with every admin by then, and reporting a failed receipt would earn a duplicate enquiry |
...
| 5.9.0.2 | One vocabulary on **four** surfaces | Step 1, the coach's card, the **add-a-coach form**, and the admin panel all say **understands** — not "reads" on one and "should your feedback be in" on another. Both halves of an intersection have to be spelled the same. ⚠️ It said "three" until 2026-09-03: the add form was missed because the role cards had taken over *editing*, so the wording never came up while looking at a coach who already existed |
...
| 5.9.8 | ~~Customer `english ` · coach `English` (case and trailing space)~~ | **Retired 2026-09-03 — unreachable, not wrong.** Both sides are radio groups now, so a stray space or a mistyped case cannot be entered by anyone. The rule still matters, because a migration or a seed could reintroduce one, so the assertion moved to `npm run simulate` — which now names the trailing space this row did. Retired because the *input path* went, not the rule; if free text ever returns, so does this |
...
| 6.6.1 | ⚠️ **The size limit holds for a coach, not just a customer.** Set the limit to 10 MB and upload a larger feedback file | Refused **before the upload starts**, naming the file's size and the limit — the customer's wording, from the customer's rule. ⚠️ A 21 MB file was accepted on 2026-09-04 and the object really landed in Blob: the client token's `maximumSizeInBytes` is not the enforcement it looks like, and only the customer's `complete` route re-checked |
...
| 6.6.5 | ⚠️ **A file between 4.5 MB and the limit, in an admin folder box** | Uploads. ⚠️ Until 2026-09-06 the folder boxes posted to a Server Action, so the bytes came through us and Vercel's ~4.5 MB body cap refused a file the setting allowed — the last upload path still routed through the server. Verified locally at 6 MB accepted, 12 MB refused against a 10 MB limit |
| 6.6.6 | ⚠️ **The dev proxy enforces the limit too.** With `STORAGE_DIR` set, upload an oversize file on each operator surface | Refused, same message. ⚠️ Only the `complete` routes checked, so the limit that held in production was absent locally — the worst place for it to differ, since local is where a limit gets tested. The customer's dev route always checked |
...
| 6.15 | **The handed-back card is a receipt.** Hand back, then read the card again | The player and focus, the hand-back date, and **all four folders** with every file and its size — the same list the admin's panel shows for that submission. It stays in its date position in the one list rather than moving to a "Submitted" section (2026-09-10). ⚠️ It said "0 files · No files on this submission" for every finished submission: the page loaded `listFilesForSubmissions`, which returns intake kinds only, then filtered it for feedback files. A filter that cannot match is the quietest kind of bug — the page rendered and stated something definite |
...
| 6.17 | **One list, newest first.** A coach with several reviews, some on the desk and some handed back | One card per submission in `submittedAt` order, newest at the top, interleaved by date. The badge — "To review" / "Awaiting review" / "Delivered ✓" — says which is which, and the card states the date the list is ordered by. ⚠️ It was two lists, "To review" above "Submitted", so a coach had to know which half a review had fallen into before they could scan for it (Ben, 2026-09-10) |
| 6.18 | ⚠️ **Assigned, but not yet sent.** Assign a coach, stop before the hand-off email, and open the coach portal | **Covered by `tests/unit/reviewCard.test.tsx`.** The card **does** appear — the row is theirs from `assigned`, by design — badged a quiet "Assigned to you", with **no files and no hand-back form**, and a line saying whose move it is: the admin's, or *"being translated first"* when it is out with a translator. ⚠️ **This row reversed on 2026-09-11.** It read "still absent from the list, as before", which I asserted without reading `isWithCoach` — a predicate that includes `assigned` deliberately. The card then rendered the customer's **originals** (`coachFileSet` is null, so `filesAsSent` falls back to them) and a live "Send for approval" the server would refuse. On a submission that needs translating, those originals are exactly what the translation exists to replace (Ben) |
| 6.18.1 | ⚠️ **The six upload doors.** As an assigned-but-unsent coach or translator, POST to each upload route directly | **409 with a reason**, not a silent accept. All six gated on `isAssignedTo` alone — true from the moment the admin *picks* someone — so the whole job could be done and only the hand-back refused. `isCoachesTurn` and `isTranslatorsTurn` are the one home per side |
| 6.18.2 | ⚠️ **The download door.** As an assigned-but-unsent coach, GET `/api/files/[id]` for an intake file | **403, "the admin hasn't sent this over to you yet"** — 403 not 404, because they *are* assigned and there is nothing to hide from them. An un-assigned operator still gets 404, which refuses to confirm the id. ⚠️ The gate is monotone (`isHandedOverFor`), so it only ever closes the window *before* the hand-off: a coach's own finished review stays downloadable, which is what the receipt on their card is made of |
| 6.18.3 | ⚠️ **The translator's mirror.** Assign a translator a leg and stop before sending | **Covered by `tests/unit/translationCard.test.tsx`.** The leg reads "Assigned to you", with no files and no upload. ⚠️ It read **"Handed back ✓"** — `open` is false both before the hand-off and after the hand-back, and the card took that one flag for "finished", so an unsent leg showed a receipt for files the translator had never been given |
| 6.18.4 | The predicates cannot drift apart | **Covered by `tests/unit/submissionStatus.test.ts`.** `isHandedOverFor` is a monotone suffix for every kind; the coach's turn is exactly `sent_to_coach`+`in_review`; `intake` is never handed to anybody. A new rung is a compile error in each exhaustive `Record` |
| 6.16 | The same on the translator's Handed back list. **Covered by `tests/integration/translatorGuards.test.ts`** | The leg's own title and date, and **its own two folders** — the one it read from and the one it delivered into. ⚠️ It showed all four folders of the submission, which is submission-scope content on a leg-scope card: hold both legs and the same block prints twice, so the history reads as a duplicate entry (2026-09-09). ⚠️ One translator may hold both legs of a submission, so the date must come from that leg's `done` rung and not a shared one |
...
| 7.6 | ⚠️ **Both legs, one translator.** Assign the same person both legs of one submission | **Covered by `tests/unit/translationCard.test.tsx`.** **One card**, with both legs as titled sections inside it, in pipeline order. Only the leg matching the current rung offers the hand-back form; the other reads as a receipt. ⚠️ It was two cards until 2026-09-10 — the honest rendering of the work, and the wrong rendering of the page: one submission appeared twice and a person scanning for a name had to read both to learn which was which (Ben). The substance the old check protected still holds, asserted *within* the card: with both legs on it, the titles are the only thing telling them apart |
| 7.7 | Each leg names its direction | **Covered by `tests/unit/translationCard.test.tsx`.** The submission is named **once**, at the top of the card; each leg's title heads its own section beneath. With both legs held, the titles are the only thing telling them apart |
...
| 7.16 | **One list, newest first.** A translator holding several submissions, some open and some handed back | **Covered by `tests/unit/translationCard.test.tsx`.** One card per submission in `submittedAt` order, newest at the top, open and finished interleaved by date. The badge says which wants action. ⚠️ It was two lists, "To translate" above "Handed back", and a translator holding both legs of one submission appeared in **both** at once (Ben, 2026-09-10) |
| 7.17 | The order comes from the query, not the page | **Covered by `tests/unit/translationCard.test.tsx`.** `legsForTranslator` already orders `submittedAt` descending; grouping preserves it, including when a submission's two legs arrive non-adjacent. A sort layered on top would be a second opinion that could disagree with the first |
...
| 7.14 | Handed-back legs stay visible | **Covered by `tests/integration/translatorGuards.test.ts`.** On the submission's own card, as a receipt with the file count they delivered. The card is badged "Handed back ✓" once every leg on it is done |
...
| 8.8 | The Download button doesn't flash a tab | **Covered by `tests/unit/downloadRow.test.ts`.** Same tab, no window opening and closing. The route answers with `Content-Disposition: attachment`, so nothing was ever going to be shown in it |
| 8.9 | ⚠️ Download still works after that change | **Covered by `tests/unit/downloadRow.test.ts`.** It is a plain `<a>`, not `ButtonLink`. `next/link` intercepts internal hrefs and was skipping this one only because `target` was set |

| 8.9.1 | ⚠️ **Two files, one row shape.** Deliver a submission with a short filename and a long one, then open the customer's download page | **Mechanism covered by `tests/unit/downloadRow.test.ts`** — a short and a long name render the identical row, nothing wraps, only the name truncates. The *look* still wants eyes. Both rows identical: name, size, Download on the right. ⚠️ The list was `flex-wrap`, so a long name pushed its button onto a second line and two files in one list looked like two designs |
| 8.9.2 | A very long filename | **Mechanism covered by `tests/unit/downloadRow.test.ts`** — a short and a long name render the identical row, nothing wraps, only the name truncates. The *look* still wants eyes. Truncates with an ellipsis. The size and the button never move; the name is the only thing that gives way |
| 8.9.3 | ⚠️ **Both customer download pages, not one.** Check the signed link from the ⑥ email *and* the status lookup | **Covered by `tests/unit/renderedLists.test.ts`.** ⚠️ It had happened again: `/feedback/[token]` hand-rolled a flat list, so a both-folder release arrived from the ⑥ email ungrouped. `FeedbackFiles` demanded a whole `FeedbackGroup` while reading only its `files` — a prop asking for more than it reads is how one of two callers gets locked out. Fixed 2026-09-08. Identical rows on both. ⚠️ Each had built this row for itself, so the first fix landed on one page and the same report came back about the other. They render one shared component now |
| 8.9.4 | No tab flash on either page | **Covered by `tests/unit/downloadRow.test.ts`.** Clicking Download starts a download in place. `target="_blank"` opened a window with nothing in it — the route answers `Content-Disposition: attachment` |
...
| 8.9.6 | Resize slowly across the whole range | **Nothing changes size continuously.** The heading and the vertical rhythm each have two fixed sizes with one step at `sm`; the column grows to its cap and holds. ⚠️ **This reverses what the check used to ask for.** It wanted continuous scaling and no snap, which was right about the *column* — one that narrows as the window widens is a fault — and wrong about type. A `vw` term gives a different size at every pixel of width, so the browser re-rasterises glyphs and em-based tracking shifts: the heading shivers and its line box drags the page below it up and down. Two attempts tried to fix that by shortening the fluid range; the range was never the problem (2026-09-10). **Covered by `tests/unit/pageColumn.test.tsx`** |
...
| 8.9.8 | Same on the `/status/[token]` link — **which comes from the payment receipt, not ⑥**. ⑥ links to `/feedback/[token]`, one submission; the receipt's link covers the address (corrected 2026-09-10) | No code prompt at all. The signed link *is* the proof: that address verified itself at step 2 and paid at step 4, and a code is single-use, so asking again costs a trip to the inbox and proves nothing. **Covered by `tests/integration/statusTokenPage.test.tsx`** |
| 8.9.9 | ⚠️ **A "Feedback ready" card leads to its files.** Click Download ↓ on the card | **Superseded by 8.9.44** — this describes the two-section page that became one list on 2026-09-03. Retire at the next reconcile. Jumps to the panel below the stack. Nothing on the card said the files were already on the page, so customers went back to their email for the link instead of scrolling |
| 8.9.10 | A card with no feedback shows no link | **Covered by `tests/unit/statusCards.test.ts`** — content and order, not layout. And a page with no downloads shows none on any card |
| 8.9.11 | ⚠️ **Both code emails look the same.** Trigger the step-2 verification code and the `/status` access code, and put them side by side | **Covered by `tests/unit/codeEmails.test.ts`.** Identical grey rounded panel, display face, same size and tracking. ⚠️ They differed in five ways: box, typeface, size, tracking and colour |
| 8.9.12 | The panel follows the shell's palette | **Covered by `tests/unit/codeEmails.test.ts`.** Its greys are the shell's `PAPER_ALT` and `LINE`, not literals, so a palette change carries the code box with it |
...
| 8.9.15 | ⚠️ **Ready before history.** Open `/status` with a released review waiting | **Superseded by 8.9.43 and 8.9.45** — this describes the two-section page that became one list on 2026-09-03. Retire at the next reconcile. The downloads are the first thing on the page; history sits under its own heading below. ⚠️ They were the other way round, so a parent whose review had just landed scrolled past their own past to reach it |
| 8.9.16 | Nothing ready | **Covered by `tests/unit/statusCards.test.ts`** — content and order, not layout. No downloads block at all, and the list is headed "Your submissions" rather than "Your history" |
...
| 8.9.19c | ⚠️ **A long filename cannot push a panel off-screen.** Open a submission with an unbroken 80-character filename and narrow the window | Everything stays inside the panel; the name truncates or wraps. ⚠️ The Coach folder box overflowed the right edge (2026-09-10). `minmax(0,…)` on the *track* is only half of it — a grid item still defaults to `min-width: auto`, and `truncate` sets `white-space: nowrap`, so the name's min-content is the whole string. A filename with spaces behaved, because its min-content is only its longest word |
| 8.9.19b | ⚠️ **A capped page never narrows as the window widens.** Drag any portal or form page across 640px and 1024px | The column grows to its cap and holds. ⚠️ Two faults, both site-wide until 2026-09-10: the gutters were subtracted *from* the cap, so `/start` went 536 → 512 → 456 in the wrong direction; and `max-w-2xl`/`max-w-3xl` lost a cascade race to `Container`'s own `max-w-[1400px]`, so the coach and translator portals rendered at 1280px while `max-w-sm` pages were correct. Which caps worked was decided by the order Tailwind emitted them in. **Covered by `tests/unit/container.test.tsx`** |
| 8.9.18 | **All five reader-facing pages resize the same way** — the three customer ones and both portals, which joined them on 2026-09-10. One shell, one cap (`max-w-3xl`), one vertical rhythm. **Covered by `tests/integration/customerShell.test.tsx`** | `/status`, `/status/[token]` and the ⑥ link share one shell. Dragging wider never narrows the column |
...
| 8.9.20 | The escape still works | "Use a different email" returns to the form and clears the old code. **An E2E step exists for this** in `e2e/goldenPath.spec.ts` (customer path, after the status lookup) — it asserts the form returns, the results go, and a fresh code field comes back empty. ⚠️ **Not yet run**: Playwright 1.62 needs macOS 14+ and the dev machine is on 13.7, so it awaits the nightly CI run or a click |
| 8.9.21 | ⚠️ **Each card identifies its own submission.** Look up an email with several | **Covered by `tests/unit/statusCards.test.ts`** — content and order, not layout. Player and age, focus, the customer's own notes, sent date and feedback-ready date. ⚠️ It was a name, a focus and a date, so two reviews of the same child were indistinguishable |
...
| 8.9.23 | ⚠️ **A card per finished review.** Look up an email with several ready | **Superseded by 8.9.43** — this describes the two-section page that became one list on 2026-09-03. Retire at the next reconcile. One green card each, headed "Ready to download (n)". ⚠️ All of them sat inside a single green panel separated only by a player name, so seven reviews arrived as one block |
| 8.9.24 | Each ready card carries the same detail as its history card | **Covered by `tests/unit/statusCards.test.ts`** — content and order, not layout. Player, age, focus, their own notes, both dates. They render one shared component, so the two lists cannot describe one submission two ways |
...
| 8.9.32 | ⚠️ **Override reads like its neighbours.** Expand a row | **Covered by `tests/unit/disclosure.test.ts`.** "This submission", "Trail" and "Override" all carry the same small-caps label and a `›` that rotates. ⚠️ Override had its own "Override…" link and a "close", so the third panel of three announced itself as a different kind of control |
| 8.9.33 | It still opens and its forms still work | **Covered by `tests/unit/disclosure.test.ts`.** It is a native `<details>` now, so it survives without hydration and cannot desync from what is on screen |
...
| 8.9.51 | **An unpaid submission counts down in hours.** Look up an email with one at `awaiting_payment` | "Finish and pay within N hours". ⚠️ This page is the only place it is ever said: an unpaid submission is **removed outright** rather than having its files cleared, and the retention emails only go out after payment. Counted from `updatedAt`, which is what the sweep reads — a declined card touches the row, so trying again buys more time |
| 8.9.52 | **A downloaded review stops saying "Feedback ready"** | **Covered by `tests/unit/statusCards.test.ts`.** The badge reads "Downloaded" at `collected` and `resolved`. ⚠️ All three released rungs read "Feedback ready", so a parent who had already downloaded was still being told to. "Ready" is an instruction |
| 8.9.53 | **A downloaded card is grey, not green** | **Covered by `tests/unit/statusCards.test.ts`.** The tint follows the badge — green only while the card wants something (`complete`, `purge_imminent`), plain once collected. The files stay on the card either way; it is the colour that changes. ⚠️ The tint keyed on *having files*, which stays true forever, so a collected review sat in green while its own badge already read "Downloaded" (Ben, 2026-09-10). Both read `wantsYou` off one constant now |
| 8.9.54 | The folder headings inside a downloaded card | Neutral. They took their green from the panel they used to sit in, and on a grey card the heading was the last green thing left |
| 8.9.40 | ⚠️ **The customer's card carries the deadline.** Look up a delivered submission on `/status` | **Covered by `tests/unit/statusCards.test.ts`** — content and order, not layout. A last line: "Files deleted in N days". ⚠️ They are told at ⑥ and again at ⑨; this is the same promise on the page they actually return to |
| 8.9.41 | And it disappears once the files are gone | **Covered by `tests/unit/statusCards.test.ts`** — content and order, not layout. A countdown after the fact is shown too late — the struck-through filenames say it better |
| 8.9.42 | **The download replaces the clock, it doesn't extend it** | **Covered by `tests/unit/deletionClock.test.ts`.** 30 days from the customer's download; the 90-day delivery backstop applies only to a customer who never downloads. ⚠️ **Reverses what this used to ask for.** It read "whichever falls later" until 2026-09-10, while `/admin/settings` labelled the knobs as an either/or, ⑥ said "30 days after you first download it", and ⑨ mailed the collection date — so a prompt collector could be warned "deleted 3 Oct" and still hold the files in December (Ben) |
| 8.9.42.1 | ⚠️ **The sweep agrees with the countdown.** Age a collected submission past `retainCollectedDays` while its delivery is recent | **Covered by `tests/integration/retentionSweep.test.ts` 9.4b and `npm run simulate`.** Warned, then purged, on its own collection clock. Under the retired rule it was months from due. Mutation-tested: reverting either query fails 16 simulate checks |
| 8.9.42.2 | ⚠️ **The backstop still covers the customer who never comes.** A delivered, never-collected submission past `retainDeliveredDays` | **Covered by 9.4c.** Warned, then purged. This is the case the backstop exists for, and the one that used to be purged in silence |
| 8.9.42.3 | **⑥ states both clocks.** Read a feedback-ready email | "We delete everything 30 days after you first download it… If you never download it, it goes after 90 days." ⚠️ It stated only the first, so the customer it was written to protect — the one who never downloads — was given no deadline until the ⑨ warning a week out, which is the surprise that line exists to prevent |
| 8.9.43 | ⚠️ **One card per submission, and only one list.** Look up an email with several finished | **Covered by `tests/unit/statusCards.test.ts`** — content and order, not layout. A single "Your submissions (n)". ⚠️ It was two sections — every finished review appeared twice, once with its files and no status, once with its status and no files. Neither card was the whole thing |
| 8.9.44 | The files are inside the card | **Covered by `tests/unit/statusCards.test.ts`** — content and order, not layout. Not in a separate panel with a "Download ↑" jump. The card already says whose review it is |
| 8.9.45 | **Newest first, and nothing re-sorts it** | The server's order stands. ⚠️ **Reverses what this used to ask for.** It put submissions with files at the top — the surviving half of the old two-section page — and that buried a submission made minutes ago under eight older ones because it had no files yet, which reads as it never having arrived (2026-09-10) |
| 8.9.46 | ⚠️ **A released submission whose files were swept.** Age one past the retention window | Listed once in its date position, reading "No longer available" |
| 8.9.47 | Nothing finished | **Covered by `tests/unit/statusCards.test.ts`** — content and order, not layout. The same single list, in the server's order |
...
| 10.6 | Two tabs, same flow | **Covered by `tests/integration/twoTabs.test.ts`.** One cookie per browser, so the newest start owns the flow — as a refresh does. The other tab is told so on its next action and returns to step 1: its code is refused **with the reason**, its uploads are refused with the reason, and it can never pay for the other tab's submission. ⚠️ Before 2026-09-10 the first tab silently acted on the second tab's row: its code was checked against the wrong submission, then waved through once that one was verified (re-verifying is a no-op), its upload was refused as "session timed out", and it would have paid for details it never entered (Ben). Every action past step 1 now says which submission the tab is on, and the server refuses when the cookie disagrees — a claim that is checked, never accepted |
| 10.6.1 | **Start over in the losing tab** | **Same test.** Leaves the winning tab's submission and cookie alone; only the losing tab resets. ⚠️ It discarded whatever the cookie named — the other tab's live submission |
| 10.6.2 | **The upload refusal in production** | The Blob client flattens every token refusal into "failed to retrieve the client token", so on step 3 the panel names both causes — window lapsed, or another tab started a new one — and points at Start over. The dev proxy and the completion step carry the exact sentence, and "Continue to payment" learns the precise reason and resets the tab |
| 10.6.3 | A lapsed window is still reported as a timeout | **Same test.** The two reasons are told apart: no cookie is "timed out", a cookie naming a different submission is "another tab" |
