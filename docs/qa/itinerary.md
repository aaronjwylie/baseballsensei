...
| 8.9.18 | All three customer pages resize the same way. **Covered by `tests/integration/customerShell.test.tsx`** | `/status`, `/status/[token]` and the ⑥ link share one shell. Dragging wider never narrows the column |
